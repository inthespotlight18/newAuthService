    ...
    public class SnipeITAuth : iGAuth
    {
        ...
        public async Task<DataTable> GetProfile()
        {
            try
            {
                ...
                //DataTable t = (DataTable)JsonConvert.DeserializeObject(jsonResponse, typeof(DataTable));

                // List<SnipeITAuth> UserList = JsonConvert.DeserializeObject<List<SnipeITAuth>>(jsonResponse);

                var dt = Helper.Tabulate(jsonResponse);
